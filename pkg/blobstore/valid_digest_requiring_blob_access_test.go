...
func TestValidDigestRequiringBlobAccessSuccess(t *testing.T) {
	...
	testSuccess := func(digest *remoteexecution.Digest, body []byte) {
		...
		blobAccess := NewValidDigestRequiringBlobAccess(bottomBlobAccess)
...
func TestValidDigestRequiringBlobAccessMalformedDigests(t *testing.T) {
	ctrl, ctx := gomock.WithContext(context.Background(), t)
	defer ctrl.Finish()
	...
	// malformed requests.
	blobAccess := NewValidDigestRequiringBlobAccess(mock.NewMockBlobAccess(ctrl))
